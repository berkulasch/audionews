...
│   │   ├── explore.tsx     # Keşfet
│   │   └── profile.tsx     # Profil
│   ├── article/[id].tsx    # Haber Detayı + Audio Player
│   └── auth/
│       ├── login.tsx       # Giriş
│       └── register.tsx    # Kayıt
├── components/
│   ├── FeaturedCard.tsx    # Öne çıkan haber kartı
│   ├── ArticleCard.tsx     # Haber kartı (horizontal/compact)
│   ├── CategoryPill.tsx    # Kategori filtresi
│   └── MiniPlayer.tsx      # Alt mini player
├── hooks/
│   └── useAudioPlayer.ts   # Audio player state yönetimi
├── lib/
│   ├── firebase.ts         # Firebase SDK init
│   ├── types.ts            # TypeScript tipleri
│   └── mockData.ts         # Test verisi
├── constants/
│   └── theme.ts            # Renkler, fontlar, spacing
└── functions/
    └── src/
        ├── index.ts        # Cloud Functions entry
        ├── newsFetcher.ts  # RSS + NewsAPI çekici
        └── ttsGenerator.ts # OpenAI TTS üreticisi
```

## Tasarım

Booksbury audiobook uygulamasından ilham alınmıştır:
- **Arka plan**: `#F5EFE1` (warm cream)
- **Vurgu**: `#BFA054` (altın)
- **Metin**: `#2F2F2F` (charcoal)
- **Font**: DM Serif Display (başlıklar)
